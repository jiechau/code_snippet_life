...
| [astro_score/astro-score_daily_full.html](astro_score/astro-score_daily_full.html) | The daily grid with the sky's own calendar under it. Same bars and same two strips, plus **three rows per cell that the forecast cannot touch**: a purple `*` when **A\* (銀心)** climbed above 10° during that half-day's dark hours, an orange `*` when **M42** did, and the **moon phase** `🌑 🌒 🌓 🌔 🌕 🌖 🌗 🌘` once per cell. Dark is the page's own −10° sun threshold, the one AstroScore already zeroes the score above; 10° rather than the horizon because below that you are looking through five or more atmospheres of haze. Because none of the three reads a cloud model they stay true at the right-hand end of the grid where the bars have given out, so a mark under a blank bar (the subject was up, the sky was hopeless) or under a no-forecast `·` is the useful case: the page answers *when is the season* beside *which night is clear*. The two marks almost never light in the same block — over a live 16-day response at 龍磐公園 all 32 blocks carried exactly one, M42 at +61°–63° in the small hours and A\* at +29°–36° in the evening — so the orange falls under a day's left glyph and the purple under its right, the same one-night-across-two-cells reading as the bars. The moon glyph is per **cell**, not per block: the elongation moves ~12° a day, under a third of the 45° each glyph covers, so there is nothing to say twice; it is drawn for local noon and is a reminder rather than an input, since AstroScore docks the score for moon altitude and knows nothing of the phase. |
