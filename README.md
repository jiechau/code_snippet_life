...
| [pure_math/](pure_math/README.md) | The astronomy behind `astro_score/`, one formula per page: galactic-core altitude, the same for M42, solar altitude, lunar altitude and phase. Demo pages only — no script and **no API call**, just numbers in and numbers out. |
| [light_pollution/](light_pollution/README.md) | How dark is the sky at a `lat,lon`? Decodes a tile of David Lorenz's World Atlas of Artificial Night Sky Brightness to SQM, Bortle and LP Zone, one point at a time or as a pannable map. Demo pages only — the tiles are static files, so there is no script and no key. |
...
All nine pages offer a third way to fill 輸入, beside typing and the GPS:
**🗺️ 在地圖上點選 (pick on the map)** opens an OpenStreetMap map over the page,
and whatever ends up under the crosshair is written into the box. It is
`pickOnMap()` in `places.js`, sitting beside `currentPosition()` — keyless, like
every other service in this repo, and with **no map library** behind it: a slippy
map is a Mercator formula, a grid of `<img>` tiles and a drag handler. Needing
neither a permission nor a secure context, it is the one of the two location
buttons that also works over `file://`.

Every page with that box also names the coordinate on a second line —
中華民國/臺北市/內湖區, from BigDataCloud's keyless `reverse-geocode-client` — which
matters most when you have just picked a point off the map and nobody typed it. The
map's tiles and that name are **helpers for the box, not data the page is about**:
they put no number, row or cell on screen, nothing computed reads them, and both
fail to nothing. That is why `pure_math/` is still "no API call" and the
`open_meteo/` pair still shows nothing but the API.
...
produced (`galactic_center.html`, `orion_nebula_m42.html`).
...
| [open_meteo/open-meteo_readable.html](open_meteo/open-meteo_readable.html) | The same request, drawn as an hour-by-hour forecast grid: a tab per model, a row per variable, colour-coded and scrollable. Both pages in this folder are deliberately **API-only** — every row is a series Open-Meteo returned, with no locally computed astronomy. |
| [astro_score/astro-score_readable.html](astro_score/astro-score_readable.html) | That grid cut down to what stargazing needs: the cloud decks plus 降雨/氣溫 (all four models fetched together, 雲量來源 tabs picking which one the grid is drawn from and 觀星 scored with, defaulting to `ecmwf_ifs025`), locally computed 太陽/月亮/月相/銀心 rows (the last being the Milky Way core's altitude, shown but not scored, with a 銀心 (max) row under it giving the fixed ceiling `90° − |緯度 + 29°|` that latitude allows), the same altitude-and-ceiling pair again for the **Orion Nebula** as M42 (°) / M42 (max) — the winter target to the core's summer one, on the same 25° airmass scale so the two read straight down a column — the site's light pollution as 光害 (SQM) / Bortle / LP Zone from [David Lorenz's World Atlas](https://djlorenz.github.io/astronomy/lp/) — the one keyless, CORS-enabled light-pollution source, read straight out of its gzipped binary tiles with `DecompressionStream`, the place the coordinates fall in (中華民國/宜蘭縣/南澳鄉/蘇澳鎮, via BigDataCloud's keyless reverse geocoder), and a 觀星 score per hour — `(100 − 雲量) × (100 − 月亮扣分) / 100`, zeroed while the sun is above −10°, where 月亮扣分 ramps 0 → 100 as the moon climbs 0° → 10°. An extra-params box prefilled `past_days=7` extends the grid backwards, visible once 從現在開始 is unticked. Loosely after `astro_score/milkyway.py`. |
| [astro_score/astro-score_daily.html](astro_score/astro-score_daily.html) | The same 觀星 score, one week at a glance: a row per place, a column per day, and each cell two block glyphs `▂▄▆█` — one per half-day (00–11, 12–23), sized by the **best** 觀星 hour in it (>90 `█`, >70 `▆`, >50 `▄`, >30 `▂`, at or below 30 blank) — each over a purple `▀` strip shaded by the block's best **MilkyScore**, `銀心 altitude° × 觀星/100` (>50, >40, >30, >20, blank at or below 20), which colours only when the sky is good *and* the galactic core is up, and an orange `▀` strip under that for the **OrionScore**, the identical product for M42 on the identical ladder. The two rarely paint together and that is the point: A* is a summer target and M42 a winter one, and even within one autumn night the core owns the evening while M42 owns the small hours — so the purple tends to fall under a day's right glyph and the orange under the next day's left one. A night straddles midnight, so it reads across two cells: one day's right glyph beside the next day's left one. The first row is 輸入, the `places.js` entry that is not a saved spot: a `lat,lon` box above Fetch, a 📍 使用目前位置 button and a 🗺️ 在地圖上點選 map picker move that one row, while the saved spots stay as the file has them; a second meta line names it (`輸入: 中華民國/新北市/烏來區`) from the same keyless BigDataCloud lookup the readable page uses. One request per place instead of one for a location box (pooled 4 at a time, since Open-Meteo 429s a burst), each returning all four forecast models; tabs pick which model the bars are scored from, defaulting to `ecmwf_ifs025` (`jma_seamless`, with its ~5 km nest over Taiwan, is one click away). An extra-params box prefilled `past_days=7` adds the week just gone as columns left of today, hidden until 從今天開始 (hide past days) is unticked — it starts ticked, so the page opens on today and the days ahead. It also fetches once on load rather than waiting for a first Fetch click. |
| [astro_score/astro-score_daily_full.html](astro_score/astro-score_daily_full.html) | The daily grid with the sky's own calendar under it. Same bars and same two strips, plus **four rows per cell that the forecast cannot touch**: three digits giving how high a body got during that half-day's dark hours in tens of degrees — purple for **A\* (銀心)**, orange for **M42**, slate for the **moon** (`6` = reached the sixties, blank = never cleared 10°) — and the **moon phase** `🌑 🌒 🌓 🌔 🌕 🌖 🌗 🌘` once per cell. Dark is the page's own −10° sun threshold, the one AstroScore already zeroes the score above; 10° rather than the horizon because below that you are looking through five or more atmospheres of haze. Because none of the four reads a cloud model they stay true at the right-hand end of the grid where the bars have given out, so a digit under a blank bar (the subject was up, the sky was hopeless) or under a no-forecast `·` is the useful case: the page answers *when is the season* beside *which night is clear*. Tens only, never two digits and never `0`, so a blank is unambiguous and a row reads along as well as down — `3 3 3 … 2 2 2` is a target going out of season week by week, which a plain presence mark could not show. **The slate digit means the opposite of the two above it** — same rule, high is bad — and since its gate is also where MoonPenalty tops out, any slate digit means at least one dark hour of that block scored zero on moonlight alone; paired with the bar it finds the moonless window inside a moonlit night, since the digit is the block's worst hour and the bar its best. The two target rows almost never both carry a digit in the same block — over a live 16-day response at 龍磐公園 all 32 blocks carried exactly one, M42 reading `6` in the small hours and A\* `3` then `2` in the evening — so the orange falls under a day's left glyph and the purple under its right, the same one-night-across-two-cells reading as the bars. The phase glyph is per **cell**, not per block: the elongation moves ~12° a day, under a third of the 45° each glyph covers, so there is nothing to say twice; it is drawn for local noon, and with the slate digit beside it makes the pair *how bright* and *how high*. |
| [astro_score/astro-bookmark.html](astro_score/astro-bookmark.html) | Not a demo of anything here: a plain list of the outside pages worth a look before driving out — CWA 月相圖, two light-pollution maps ([lightpollutionmap.info](https://www.lightpollutionmap.info/), [lightpollutionmap.app](https://lightpollutionmap.app/zh-tw/)), NCDR 即時降雨預警, and CWA 雷達回波圖 / 一週 / 降水預報. No form, no request, no script; it shares none of the demo pages' machinery. |
...
| [pure_math/orion_nebula_m42.html](pure_math/orion_nebula_m42.html) | The `M42 (°)` and `M42 (max)` rows of `astro-score_readable` — and the altitude behind the daily grid's orange strip — on their own, the same way `galactic_center.html` shows the `銀心` pair. It **is** that page with one pair of constants changed (RA 05h35m17.3s, Dec −05°23′28″ in place of Sagittarius A*), which is most of the point: a fixed equatorial position needs no ephemeris, so swapping the pair is the whole port. At δ = −5° the winter target rides ~59°–63° over Taiwan where the core reaches only ~36°–39°. The one formula in this folder with **no `milkyway.py` counterpart** to check against. **No API call.** |
...
| [light_pollution/light_pollution_map.html](light_pollution/light_pollution_map.html) | The same atlas as a **map**: Lorenz's sky-brightness layers (2016–2025, plus his 2013–2025 trend) or NASA GIBS' VIIRS night lights — the 2012 and 2016 Black Marble composites and a **nightly** NOAA-20 layer with a date box — over a choice of basemap, with an opacity slider and the atlas's 15-step colour key. **Click anywhere and it reads that point**: SQM / Bortle / LP Zone from the *binary* tile `binary-tile.html` decodes. Past zoom 8, where the rendered tiles run out, the page **paints the overlay itself from those same binary tiles**, so the colour and the number are the same numbers and a deep zoom shows the real 30-arcsec grid instead of a smear. Saved spots, 📍 使用目前位置 and a shareable `#lat=…&lon=…&z=…` link. No key and no map library: the slippy map is the Mercator pair and a grid of `<img>` tiles already in `places.js`. |
