...
holds the saved stargazing spots (瑞光路, 大崙頭山, 大武崙砲台, 內洞停車場, 烏石港,
...
right now, and every page has a **使用目前位置 (use current position)** button that
points it at the device's own GPS. Keeping it in the shared list rather than in a
page is what gives the one page with no location box —
[astro-score_daily.html](astro_score/astro-score_daily.html), whose rows *are* the
places — a "here" row: the button moves 輸入 and the row follows. Geolocation needs
